...
# Variable Binding Mapping Loops
...
# Native Offline Directory Picker Wrapper Function
...
# Real-Time UI Console Output Handler
...
$BtnLaunch.Add_Click({
    ...
    Start-Job -ScriptBlock {
        ...
        if (-not (Test-Path $PaExe) -or -not (Test-Path $AxExe)) {
            ...
            Write-ToLocalLog "CRITICAL: Engine executables missing. PA: $PaExe | Axiom: $AxExe"
            return
        }
        ...
        if ($PaDisk.FreeSpace -lt $MinSpaceBytes -or $AxDisk.FreeSpace -lt $MinSpaceBytes) {
            $PaGB = [Math]::Round($PaDisk.FreeSpace / 1GB, 2)
            $AxGB = [Math]::Round($AxDisk.FreeSpace / 1GB, 2)
            Send-Msg "[!] CRITICAL CAPACITY ERROR: Target drive space insufficient. Minimum 500 GB required."
            Send-Msg "    -> Drive $PaDrive ($PaGB GB Free) | Drive $AxDrive ($AxGB GB Free)"
            Write-ToLocalLog "ABORTED: Insufficient storage. PA Drive Free: $PaGB GB, Axiom Drive Free: $AxGB GB"
            return
        }

        $UfdFiles = Get-ChildItem -Path $Src -Filter "*.ufd"
        if ($UfdFiles.Count -eq 0) {
            Send-Msg "[!] Pipeline halted: Zero (.ufd) metadata collections found in the designated source root."
            return
        }

        Send-Msg "[+] Workstation space confirmed. Launching parallel processing threads for $($UfdFiles.Count) mobile cases."
        Write-ToLocalLog "Started dual processing run for $($UfdFiles.Count) devices."

        foreach ($File in $UfdFiles) {
            $CaseName = $File.BaseName
            Send-Msg ">>> Spawning Dual Processing Nodes for Case File: $CaseName <<<"
            Write-ToLocalLog "Processing Case Item: $CaseName"
            ...
            Send-Msg "    [-] Inseyets Engine Node Online: Process ID ($($ProcPA.Id))"
            Send-Msg "    [-] Magnet Axiom Engine Node Online: Process ID ($($ProcAX.Id))"
            Send-Msg "[...] Awaiting extraction analysis blocks (This cycle may process for several hours)..."

            while (-not $ProcPA.HasExited -or -not $ProcAX.HasExited) {
                Start-Sleep -Seconds 10
            }

            Send-Msg "[✓] Dual-processing analysis completed cleanly for case profile: $CaseName"
            Write-ToLocalLog "Completed processing loop for case profile: $CaseName"
        }
        Send-Msg "🎉 FORENSIC PIPELINE SUCCESS: All queued items have completed extraction analysis."
        Write-ToLocalLog "Pipeline execution run terminated successfully."
    } -ArgumentList $TxtSrc.Text, $TxtPaOut.Text, $TxtAxOut.Text, $TxtPaExe.Text, $TxtAxExe.Text | Out-Null

    $UIWatcher = New-Object System.Windows.Threading.DispatcherTimer
    $UIWatcher.Interval = [TimeSpan]::FromSeconds(1)
    $UIWatcher.Add_Tick({
        $Job = Get-Job | Select-Object -Last 1
        if ($Job) {
            $ConsoleStream = Receive-Job -Job $Job
            foreach ($Line in $ConsoleStream) { if ($Line) { Write-PipelineConsole $Line } }
            if ($Job.State -ne "Running") {
                $UIWatcher.Stop()
                $BtnLaunch.IsEnabled = $true
                Remove-Job -Job $Job
                $LblDiskStatus.Content = "Pipeline Complete - Operational Framework Idle"
            } else {
                $LblDiskStatus.Content = "Processing Background Tasks Parallel Queue Running..."
            }
        }
    })
    $UIWatcher.Start()
